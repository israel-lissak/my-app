import { withForm } from "./form";
import { sliceFormOpts } from "./formOptions";

/**
 * RecipesField component for managing an array of recipes.
 * @param form - The form instance to use for field management
 * @returns A form field component for managing recipes array
 */
export const RecipesFields = withForm({
	...sliceFormOpts,
	render: ({ form }) => {
		return (
			<form.Field name="recipes" mode="array">
				{(field: any) => {
					const recipes = field.state.value as any[];
					return (
						<div>
							{recipes.map((_: any, i: number) => {
								return (
									<form.Field key={i} name={`recipes[${i}]`}>
										{(subField: any) => {
											const recipe = subField.state.value as any;
											return (
												<div className="flex flex-col gap-4 border-2 border-gray-300 p-4 rounded-md mb-2">
													<div className="flex flex-row items-center justify-between">
														<button
															type="button"
															className="w-min px-4 bg-red-500 text-white rounded"
															onClick={() => field.removeValue(i)}
															disabled={recipes.length === 1}
															title={
																recipes.length === 1
																	? "At least one recipe required"
																	: "Remove recipe"
															}
														>
															-
														</button>
														<h3 className="text-xl font-semibold">
															Recipe {i + 1}
														</h3>
														<div></div>
													</div>

													<form.AppField
														name={`recipes[${i}].kind`}
														children={(field: any) => (
															<field.SelectField
																label="Kind:"
																options={["cake", "pancake"]}
																getOptionLabel={(option: any) =>
																	option as string
																}
																onValueChange={(value: any) => {
																	if (value === "cake") {
																		subField.handleChange({
																			kind: "cake",
																			eggs_count: 0,
																		});
																	} else if (value === "pancake") {
																		subField.handleChange({
																			kind: "pancake",
																			radius: 0,
																			has_syrup: false,
																		});
																	}
																}}
															/>
														)}
													/>

													{recipe?.kind === "cake" && (
														<form.AppField
															name={`recipes[${i}].eggs_count`}
															children={(field: any) => (
																<field.NumberField label="Eggs Count:" />
															)}
														/>
													)}

													{recipe?.kind === "pancake" && (
														<>
															<form.AppField
																name={`recipes[${i}].radius`}
																children={(field: any) => (
																	<field.NumberField label="Radius:" />
																)}
															/>

															<form.AppField
																name={`recipes[${i}].has_syrup`}
																children={(field: any) => (
																	<field.CheckboxField label="Has Syrup?" />
																)}
															/>
														</>
													)}
												</div>
											);
										}}
									</form.Field>
								);
							})}
							<button
								className="mt-2 px-4 py-2 bg-blue-500 text-white rounded"
								onClick={() => field.pushValue({})}
								type="button"
							>
								Add Recipe
							</button>
						</div>
					);
				}}
			</form.Field>
		);
	},
});
